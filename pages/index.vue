...
function vote(choice) {
  if (!loggedIn.value) return alert('You need to login to vote')
  $fetch('/api/vote', {
    method: 'POST',
    body: { choice }
  }).then(refresh)
...
<template>
  <div class="min-h-screen flex flex-col gap-8 items-center pt-4">
    <h1 class="text-3xl">Coffee and Tea?</h1>
    ...
    <div>
      ...
      <a v-else href="/api/auth" class="bg-black text-white px-4 py-2 rounded">Login with GitHub to vote</a>
    </div>
    <ul>
